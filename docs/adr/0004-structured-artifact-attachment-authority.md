...
Artifact reference resolution preserves authored Markdown and HTML instead of replacing source text. Citation cleanup and public-source projection prepare Markdown publication bytes once, after the root attachment's original content digest has been checked and before dependency discovery, binding, and admission budgets. Both operate only on citation tokens with exact original source spans. Markdown links, numeric reference labels, bracketed filenames and code remain intact, so Artifact and Evidence links need no post-cleanup identity repair. Staging stores the validated bytes without running citation preparation again.
