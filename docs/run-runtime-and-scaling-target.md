...
> Accepted and implemented. The bounded local control-plane evidence and its limitations are captured under [Captured local load evidence](#captured-local-load-evidence) and regenerated by `make validate-runtime`.
...
One frozen single-process, single-database campaign (2026-09-08; Apple arm64,
16 logical CPUs; Python 3.14.7 and PostgreSQL 18.6) submitted 10,000 Query Runs
(1,000 Retrieval over 1/10/50/100 Workspaces, 4,000 Fast, 5,000 Research) plus
1,000 Corpus Mutation Runs spread FIFO over 100 Workspaces to fake executors that
held exactly 16 Query and two mutation local slots. It passed: 11,000 accepted
and 11,000 terminal rows, no lost Run and no duplicate terminal, 70.654 s,
155.689 Runs/s, 39.924 s drain, and one pre-insert rejection at the mutation
admission limit. Control operations stayed bounded — accept p50/p95 2.6/3.4 ms,
