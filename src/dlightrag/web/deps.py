...
def _citation_badges(text: str) -> Markup:
    ...
    The trailing ``### References`` section generated by the LLM is stripped
    because the web UI already surfaces sources via interactive citation
    badges and the sources panel.
    """
    from dlightrag.citations.parser import DOC_CITATION_PATTERN

    # Strip the LLM-generated References section — redundant in the web UI
    cleaned = _REFERENCES_SECTION.sub("", str(text)).rstrip()
