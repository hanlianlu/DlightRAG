...
def _make_md() -> MarkdownIt:
    """Attach browser-only rendering to the shared answer grammar."""
    md = answer_markdown()
    md.options["highlight"] = _highlight_fn
...
_md_chunk.inline.ruler.before("escape", "math_inline", math_inline_rule)
...
def _place_resources(
    ...
    HTML remains disabled. Link labels, code and math retain their parser roles.
    """
    for block in blocks:
