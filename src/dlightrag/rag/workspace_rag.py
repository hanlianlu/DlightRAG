...
from dlightrag.rag.ingestion.engine import PreparedIngestFile, UnifiedIngestionEngine
...
from dlightrag.rag.rerank import build_rerank_func, rerank_consumes_images
...
if TYPE_CHECKING:
    from dlightrag.rag.ingestion.document_embedding import RobustDocumentEmbedder
    from dlightrag.rag.lightrag_stores import LightRAGStores
    from dlightrag.rag.ports import BM25Search, RetrievalBackend
    from dlightrag.rag.retrieval.lightrag_backend import LightRAGMixBackend
    from dlightrag.rag.retrieval.provenance import ProvenanceCache
...
class WorkspaceRag:
    ...
    async def _do_initialize(self) -> None:
        ...
        from dlightrag.rag._lightrag_patches import apply as apply_lightrag_patches
    ...
    async def _do_initialize_unified(self) -> None:
        ...
        if lightrag.chunks_vdb is not None:
            from dlightrag.rag.retrieval.filtering import FilteredVectorStorage
    ...
    async def areset(
        ...
        *,
        ...
        from dlightrag.rag.reset import areset
    ...
    async def _purge_existing_for_replace(
        ...
        *,
        ...
        from dlightrag.rag.ingestion.cleanup import cascade_delete, collect_deletion_context
    ...
    async def aretrieve(
        ...
        **kwargs: Any,
        ...
        if stores is not None:
            from dlightrag.rag.retrieval.provenance import (
                ProvenanceCache,
        ...
        # as [ref-chunk_idx].
        from dlightrag.rag.retrieval.references import (
    ...
    async def aupdate_metadata(
        ...
        from dlightrag.rag.retrieval.metadata_fields import normalize_user_metadata
