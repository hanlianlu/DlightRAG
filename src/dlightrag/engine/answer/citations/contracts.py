...
CITATION_TRAILING_BOUNDARY = r"(?![A-Za-z0-9_-])"

CITATION_PATTERN = re.compile(
    rf"\[({_CHUNK_REFERENCE_ID})-({_CHUNK_INDEX})\]{CITATION_TRAILING_BOUNDARY}"
)
DOC_CITATION_PATTERN = re.compile(
    rf"\[((?:{_NUMERIC_REF}|{_ATTACHMENT_REF}))\]{CITATION_TRAILING_BOUNDARY}"
)
