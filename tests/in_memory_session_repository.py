...
"""The in-memory Agent Session adapter and the fixture subclass over it.

The adapter lives beside its tests rather than in the Engine package: nothing in
`src/` composes it, and its job is to mirror the PostgreSQL adapter's CAS
semantics for tests that cannot open a database.
"""
