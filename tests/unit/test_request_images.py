...
"""Tests for VLM-assisted query image description."""
...
def _describer(vlm, *, max_images: int, max_bytes: int = 10_000, max_px: int = 64):
...
async def test_query_image_describer_returns_descriptions() -> None:
    vlm = AsyncMock(return_value="a line chart about revenue")
    describer = _describer(vlm, max_images=1)
...
async def test_query_image_describer_without_vlm_returns_empty() -> None:
    vlm = AsyncMock()
    describer = _describer(None, max_images=1)
...
async def test_query_image_describer_describes_images_concurrently() -> None:
    ...
    describer = _describer(vlm, max_images=3)

    descriptions = await describer.describe([_image_block(), _image_block(), _image_block()])
...
async def test_query_image_compression_skip_preserves_sparse_ordinal_and_sibling() -> None:
    ...
    describer = _describer(vlm, max_images=2, max_bytes=100, max_px=128)
